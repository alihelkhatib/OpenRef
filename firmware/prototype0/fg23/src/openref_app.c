...
#endif
#ifdef OPENREF_APP_AUDIO_LINK_FG23
#include "openref_audio_link_fg23_sdk.h"
#endif

#if defined(OPENREF_APP_AUTOTX) || defined(OPENREF_APP_AUTORX) || defined(OPENREF_APP_AUTOROLE) || defined(OPENREF_APP_NETWORK)
...
#ifdef OPENREF_APP_AUDIO_LINK_FG23
static openref_audio_fg23_t app_audio_link;
#endif

#ifdef OPENREF_APP_WATCHDOG
#define OPENREF_WATCHDOG_MAIN_TASK 0u
static openref_watchdog_fg23_t app_watchdog;
static bool app_watchdog_ready;
static bool app_watchdog_fault_reported;
#endif
...
#endif
#ifdef OPENREF_APP_AUDIO_LINK_FG23
    printf("\r\n{{(openrefAudioLink)}{Status:%s}{Bus:EUSART1}{BitRate:8000000}}}\r\n",
           openref_audio_fg23_sdk_init(&app_audio_link) ? "Armed" : "InitFail");
#endif
#ifdef OPENREF_APP_WATCHDOG
    const openref_watchdog_fg23_config_t watchdog_config = {
        .gate = {
            .required_mask = 1u << OPENREF_WATCHDOG_MAIN_TASK,
            .startup_grace_ms = 1000u,
            .task_timeout_ms = {500u},
        },
        .hardware_timeout_ms = 2000u,
    };
    app_watchdog_ready = openref_watchdog_fg23_init(
        &app_watchdog, &watchdog_config, openref_watchdog_fg23_sdk_hooks());
    printf("\r\n{{(openrefWatchdog)}{Status:%s}{TimeoutMs:%u}}}\r\n",
           app_watchdog_ready ? "Armed" : "InitFail",
           (unsigned int)watchdog_config.hardware_timeout_ms);
#endif
}
...
{
#ifdef OPENREF_APP_AUDIO_LINK_FG23
    openref_audio_fg23_sdk_process();
#endif
#ifdef OPENREF_APP_WATCHDOG
    bool watchdog_ok = app_watchdog_ready &&
        openref_watchdog_fg23_report(&app_watchdog, OPENREF_WATCHDOG_MAIN_TASK) &&
        openref_watchdog_fg23_process(&app_watchdog);
    if (!watchdog_ok && !app_watchdog_fault_reported) {
        printf("\r\n{{(openrefWatchdog)}{Status:FeedWithheld}{FaultMask:%lu}}}\r\n",
               (unsigned long)app_watchdog.gate.last_fault_mask);
        app_watchdog_fault_reported = true;
    } else if (watchdog_ok) {
        app_watchdog_fault_reported = false;
    }
#endif
#ifdef OPENREF_APP_NETWORK
