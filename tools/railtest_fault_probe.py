...
def run_fault_probe(
    *,
    ...
    log_path = output_dir / f"{date}-railtest-fault-probe.log"
    ...
    with serial.Serial(port, baudrate=baud, timeout=0.1) as handle:
        ...
        thread.start()
        for command in ("rx 0", "resetCounters", "setNotifications 1"):
            write_command(handle, command)
            time.sleep(0.2)
    ...
    summary = {
        ...
        "port": port,
        "rf_path": rf_path,
        "rf_state": status.get("RfState"),
        "rail_state_active": status.get("RAIL_state_active"),
        "rail_state_rx": status.get("RAIL_state_rx"),
        "rail_state_tx": status.get("RAIL_state_tx"),
        ...
        "rx_overflow": int(status.get("RxOverflow", "0")),
        "frame_errors": int(status.get("FrameErrors", "0")),
        "log": str(log_path),
    }
    ...
    ok = (
        summary["rf_state"] == "Idle"
        and summary["rail_state_active"] == "1"
        and summary["rail_state_rx"] == "0"
        and summary["rail_state_tx"] == "0"
    )
    return 0 if ok else 1


def main() -> None:
    parser = argparse.ArgumentParser(description="Probe safe RAILtest fault recovery.")
