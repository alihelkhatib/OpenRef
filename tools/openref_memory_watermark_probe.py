...
from railtest_openref_packet_run import HEADER_BYTES, build_openref_packet, payload_commands
...
ROLE_MEM_REPORT = 4
MEM_RE = re.compile(
    r"\{\{\(openrefMem\)\}\{StateBytes:(?P<state_bytes>\d+)\}"
    r"\{MaxPacketBytes:(?P<max_packet_bytes>\d+)\}\{Rx:(?P<rx>\d+)\}"
    r"\{Gaps:(?P<gaps>\d+)\}\{Faults:(?P<faults>\d+)\}"
)


def parse_mem_markers(text: str) -> list[dict[str, int]]:
    return [
...
def run_memory_watermark_probe(
    *,
    rx_port: str,
    tx_port: str,
    output_dir: Path,
    ...
    rx_log = output_dir / f"{run_id}-openref-memory-watermark-{rx_port.lower()}-rx.log"
    tx_log = output_dir / f"{run_id}-openref-memory-watermark-{tx_port.lower()}-tx.log"
    ...
    with serial.Serial(rx_port, baudrate=baud, timeout=0.1) as rx_handle:
        with serial.Serial(tx_port, baudrate=baud, timeout=0.1) as tx_handle:
            ...
            write_command(rx_handle, f"setmemw 0x{role_address:08x} {ROLE_MEM_REPORT}")
            time.sleep(0.5)
            ...
            time.sleep(0.5)
            write_command(rx_handle, f"setmemw 0x{role_address:08x} {ROLE_IDLE}")
            time.sleep(0.3)
    ...
    mem_markers = parse_mem_markers(rx_text)
    pre = mem_markers[0] if mem_markers else None
    post = mem_markers[-1] if len(mem_markers) >= 2 else None
    autorx_markers = parse_autorx_markers(rx_text)
    last_rx = autorx_markers[-1] if autorx_markers else None
    counters = parse_status_counters(rx_text)
    tx_command_overflow = "Input buffer is FULL" in tx_text
    passed = bool(
        pre
        and post
        and pre["state_bytes"] == post["state_bytes"]
        and pre["max_packet_bytes"] == post["max_packet_bytes"]
        and post["rx"] >= packets
        and post["gaps"] == 0
        and post["faults"] == 0
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Capture OpenRef AutoRole fixed memory footprint before and after RX traffic."
    )
    ...
    parser.add_argument("--command-delay-seconds", type=float, default=0.15)
    ...
    if role_address is None:
        role_address = resolve_named_symbol_address(nm_path=args.nm, elf_path=args.elf, symbol=ROLE_SYMBOL)
