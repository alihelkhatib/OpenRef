...
class SharedMedium:
    ...
    def __init__(self, simulation: Simulation, propagation_delay_us: int = 2, *, random_seed: int = 1, base_loss: float = 0.0) -> None:
    ...
    def transmit(self, packet: Packet, airtime_us: int, on_complete=None) -> Transmission:
        ...
        for active in self.active:
            if active.end_us > tx.start_us:
                ...
                tx.collided = True
                self.simulation.record("collision_detected", node_id=packet.source_id, packet_id=packet.packet_id, with_packet_id=active.packet.packet_id)
        self.active.append(tx)
        self.simulation.record("tx_start", node_id=packet.source_id, packet_id=packet.packet_id, airtime_us=airtime_us, kind=packet.kind.value)
        ...
        def finish() -> None:
            ...
            if tx.collided:
                reason = "collision"
                self.simulation.record("packet_collided", node_id=packet.source_id, packet_id=packet.packet_id, kind=packet.kind.value)
            elif self.random.random() < self._loss_probability(tx.start_us):
                reason = "random_loss"
                self.simulation.record("packet_lost", node_id=packet.source_id, packet_id=packet.packet_id, kind=packet.kind.value)
            else:
                delivered = True
                latency = self.simulation.now_us + self.propagation_delay_us - packet.created_at_us
                self.simulation.record("packet_delivered", node_id=packet.source_id, packet_id=packet.packet_id, latency_us=latency, kind=packet.kind.value)
